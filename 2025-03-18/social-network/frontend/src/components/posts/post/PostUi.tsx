...
export default function PostsUi({
  ...
  // Parse the image URL(s) from the post.
  const getImages = (): string[] => {
    let images: string[] = [];
    if (post.imageUrl) {
      try {
        // If post.imageUrl is a JSON string (representing an array), parse it.
    ...
    <div className="border rounded-lg shadow-lg bg-white p-6 mb-6">
      ...
      {!isEditing ? (
        <div className="mt-4">
          ...
          {/* Display carousel if there is image data */}
          {post.imageUrl && (
            <div className="mb-4">
              <ImageCarousel images={getImages()} />
            </div>
          )}
          <div
