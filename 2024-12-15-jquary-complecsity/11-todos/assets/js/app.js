...
(() => {
    ...
    const generateUsersTable = todos => {
        ...
        const userTodosMap = userIds.map(userId => {
            const userTodos = todos.filter(todo => todo.userId === userId);
            const completed = userTodos.filter(todo => todo.completed).length;
            const incompleted = userTodos.length - completed;
            return { userId, completed, incompleted };
        });
    
        const result = userTodosMap.map(userTodos => `
            <tr>
